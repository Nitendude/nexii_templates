...
@section('content')
    @php
        ...
        $documentType = old('document_type', $documentType ?? ($baseData['document_type'] ?? 'service_invoice'));
    ...
    <div class="d-flex align-items-center justify-content-between mb-3">
        ...
        <a class="btn btn-outline-secondary" href="{{ route('billing.service-invoices') }}">Back</a>
    </div>

    <div class="d-flex flex-wrap align-items-center gap-2 mb-3">
        <a class="btn btn-primary" href="{{ route('billing.service-invoices.create', ['job_order_id' => $joId]) }}">Create New</a>
        <div class="btn-group">
    ...
    <div class="eh-card p-4">
        ...
        <form method="POST" action="{{ $isEdit ? route('billing.service-invoices.update', $statement) : route('billing.service-invoices.store') }}" enctype="multipart/form-data">
            @csrf
            @if($isEdit)
                @method('PUT')
            @endif
            ...
            <input type="hidden" name="document_type" id="documentTypeInput" value="{{ $documentType ?? 'service_invoice' }}">
            <input type="hidden" name="deduct_advances" value="0">
            <div class="alert alert-warning d-flex flex-wrap align-items-center justify-content-between gap-2 mb-3">
                <div>
                    <div class="fw-semibold">Advance Deduction</div>
                    <div class="small mb-0">Keep this checked if this service invoice should deduct the JO advances. Uncheck it if the client does not want advances deducted.</div>
                </div>
            ...
            <div class="mt-4">
                ...
                <div class="text-muted small mt-1">Scan the document first, then upload the saved file here. Accepted files: PDF, JPG, JPEG, PNG, WEBP, BMP, TIF, TIFF. Max 20MB each. Uploaded files are stored in APM and included with the downloadable service invoice package when possible.</div>
            ...
            <div class="d-flex flex-wrap justify-content-end gap-2 mt-4">
                <button
                    ...
                    type="submit"
                    formtarget="_blank"
                    formaction="{{ route('billing.service-invoices.draft') }}"
                    id="draftDocumentBtn"
                >
                    Print Draft
                </button>
...
<script>
    (function () {
        ...
        const linkEnergiesServiceTemplateRows = [
            { description: 'A. SERVICE CHARGES' },
            { description: 'BROKERAGE FEE' },
            { description: 'VAT 12%' },
            { description: 'LESS: 10% EWT' },
            { description: 'B. OTHER SERVICES OF CUSTOMS BROKER' },
            { description: 'HANDLING FEE', amount: '1500.00' },
            { description: 'DOCUMENTATION', amount: '2500.00' },
            { description: 'PROCESSING EXPENSES (1ST CTNR)', amount: '4500.00' },
            { description: 'XRAY; ESS', amount: '2500.00' },
            { description: 'VAT 12%' },
            { description: 'LESS: 2% EWT' },
        ];
        const petPlusNonReceiptedTemplates = [
            'CUSTOMS FORMS;STAMPS & NOTARIAL',
            'DOCUMENTATION & HANDLING FEE',
            'PROCESSING EXPENSES',
            "ADD'L PROCESSING - ASSESSMENT",
            "BAI INSPECTOR'S FEE",
            'X-RAY FIELD & OTHER EXPENSES',
        ];
        ...
        const applyLinkEnergiesServiceTemplate = () => {
            const siBody = document.getElementById('siItemsBody');
            if (!siBody) {
                return;
            }
            ...
            siBody.innerHTML = '';
            linkEnergiesServiceTemplateRows.forEach((rowData) => {
                const row = document.createElement('tr');
                row.innerHTML = siRowHtml({
        ...
        const applyClientMode = () => {
            if (isConverge()) {
                ...
                if (isPetPlus()) {
                    ...
                    applyLinkEnergiesServiceTemplate();
        ...
        document.getElementById('btnBillingStatement')?.addEventListener('click', (event) => {
            ...
            const target = '{{ route('billing.create') }}' + (joId ? ('?job_order_id=' + encodeURIComponent(joId)) : '');
