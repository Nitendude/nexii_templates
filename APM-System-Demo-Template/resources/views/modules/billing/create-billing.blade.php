...
@section('content')
    @php
        ...
        $existingServiceUrl = !empty($existingServiceInvoice) ? route('billing.service-invoices.show', $existingServiceInvoice) : null;
        $getValue = function (string $key, $fallback = '') use ($baseData) {
            return old($key, $baseData[$key] ?? $fallback);
        };
        $deductAdvancesValue = old('deduct_advances', $baseData['deduct_advances'] ?? '1');
        $deductAdvancesChecked = filter_var($deductAdvancesValue, FILTER_VALIDATE_BOOLEAN);
    ...
    @if(($existingBillingStatement ?? null) || ($existingServiceInvoice ?? null))
        <div class="alert alert-info d-flex flex-wrap align-items-center gap-2 mb-3">
            ...
            @if($existingServiceInvoice ?? null)
                <a class="btn btn-sm btn-outline-primary" href="{{ route('billing.service-invoices.show', $existingServiceInvoice) }}">
    ...
    <div class="eh-card p-4">
        ...
        <div id="connectivityStatusBanner" class="alert alert-success d-none mb-3" role="status" aria-live="polite"></div>

        <form method="POST" action="{{ $isEdit ? route('billing.update', $statement) : route('billing.store') }}" enctype="multipart/form-data">
            @csrf
            @if($isEdit)
                @method('PUT')
            @endif
            ...
            <div class="alert alert-warning d-flex flex-wrap align-items-center justify-content-between gap-2 mb-3">
                <div>
                    ...
                    <div class="small mb-0">Keep this checked if this billing should deduct the JO advances. Uncheck it if the client does not want advances deducted.</div>
                </div>
                ...
                <div class="table-responsive">
                    <table class="table table-bordered align-middle">
                        ...
                        <tbody id="siItemsBody">
                            @for($i = 0; $i < $siRows; $i++)
                                @php
                                    $siDescValue = (string) ($siDescSeed[$i] ?? '');
                                    $siUpper = strtoupper(trim($siDescValue));
                                    $isSiSectionHeader = in_array($siUpper, ['A. SERVICE CHARGES', 'B. OTHER SERVICES OF CUSTOMS BROKER'], true);
                                @endphp
                                <tr>
                                    <td><input class="form-control" name="si_item_description[]" value="{{ $siDescSeed[$i] ?? '' }}"></td>
                                    <input type="hidden" name="si_quantity[]" value="{{ $siQtySeed[$i] ?? '' }}">
                                    <input type="hidden" name="si_unit_cost[]" value="{{ $siUnitSeed[$i] ?? '' }}">
                                    @if($isSiSectionHeader)
                                        <td>
                                            <input type="hidden" name="si_amount[]" value="0">
            ...
            <div class="mt-4">
                ...
                <div class="text-muted small mt-1">Scan the document first, then upload the saved file here. Accepted files: PDF, JPG, JPEG, PNG, WEBP, BMP, TIF, TIFF. Max 20MB each. Uploaded files are stored in APM and included with the downloadable billing package when possible.</div>
                @if($isEdit && $statement?->attachments?->count())
                    <div class="mt-2 small">
                        <span class="fw-semibold">Current attachments:</span>
                        @foreach($statement->attachments as $attachment)
                            <span class="badge text-bg-light border">{{ $attachment->filename }}</span>
                        @endforeach
                    </div>
                @endif
            </div>
            ...
            <div class="d-flex flex-wrap justify-content-end gap-2 mt-4">
                <button
                    ...
                    type="submit"
                    formtarget="_blank"
                    formaction="{{ route('billing.draft') }}"
                    id="draftDocumentBtn"
                >
                    Print Draft
                </button>
...
<script>
    (function () {
        ...
        const billingForm = document.querySelector('form[enctype="multipart/form-data"]');
        const draftButton = document.getElementById('draftDocumentBtn');
        const connectivityBanner = document.getElementById('connectivityStatusBanner');
        const offlineQueueKey = `apm.billing.offline.queue:${window.location.pathname}${window.location.search}`;

        const setConnectivityBanner = (isOnline, message = '') => {
            if (!connectivityBanner) return;
            connectivityBanner.classList.remove('d-none', 'alert-success', 'alert-warning', 'alert-info');
            if (isOnline) {
                connectivityBanner.classList.add('alert-success');
                connectivityBanner.textContent = message || 'Online: Changes can be saved to the server.';
            } else {
                connectivityBanner.classList.add('alert-warning');
                connectivityBanner.textContent = message || 'Offline: You can keep editing. Save will queue and auto-submit once internet is back.';
            }
        };

        const getQueuedPayload = () => {
            try {
        ...
        const setQueuedPayload = (payload) => {
            localStorage.setItem(offlineQueueKey, JSON.stringify(payload));
        };

        const clearQueuedPayload = () => {
        ...
        const restoreFormSnapshot = (form, snapshot) => {
        ...
        const submitQueuedWhenOnline = () => {
            if (!navigator.onLine || !billingForm) return;
            const queued = getQueuedPayload();
            if (!queued) return;

            restoreFormSnapshot(billingForm, queued.formData);
            if (typeof refreshTotals === 'function') {
                refreshTotals();
            }
            clearQueuedPayload();
            setConnectivityBanner(true, 'Online: Reconnected. Submitting your queued save now...');
            billingForm.action = queued.action || billingForm.action;
            billingForm.method = queued.method || 'POST';
            billingForm.target = queued.target || '';
            billingForm.submit();
        };
        ...
        const kiltonClarkNonReceiptedTemplates = [
            { description: 'CUSTOMS FORMS,STAMPS & NOTARIAL', amount: '1000.00' },
            { description: 'DOCUMENTATION & HANDLING', amount: '2500.00' },
            { description: 'PROCESSING EXPENSES-MANILA', amount: '4500.00' },
            { description: 'PROCESSING EXPENSES-CLARK', amount: '4500.00' },
            { description: "ADDT'L PROCESSING - ESS, CIIS, POLICE , ASSESSMENT" },
            { description: "ADDT'L PROCESSING - EXAMINATION EXPENSES" },
        ];
        const kiltonClarkReceiptedTemplates = [
            'WHARFAGE FEE - PPA SI# EI-MICT-000000558534',
            'ARRASTRE CHARGES - ICTSI CSI# 659192',
            'CTNR. EQUIPMENT CLEARANCE - AISL OR# G003041025103227147',
            'ETRAC - ASCENT INV# 076027',
            'BOC OR# 01942493620',
            'ICC APPLICATION FEE - DTI OR# 07360303',
            'TRUCKING/DELIVERY CHARGES - CLARK,PAMPANGA',
            'TABS ( TERMINAL APPOINTMENT BOOKING SYSTEM)',
            'EMPTY RETURN FEE',
        ];
        const kiltonClarkServiceTemplates = [
            'BROKERAGE FEE',
            'VAT 12%',
        ];
        const allianceNonReceiptedTemplates = [
            { description: 'CUSTOMS FORMS & STAMPS', amount: '465.00' },
            { description: 'DOCUMENTATION & PHOTOCOPY', amount: '2500.00' },
            { description: 'NOTARIAL FEE & INTERCOMMERCE CHARGE', amount: '165.00' },
            { description: 'PROCESSING EXPENSES', amount: '4500.00' },
            { description: 'HANDLING FEE', amount: '1500.00' },
            { description: "ADDT'L PROCESSING - ASSESSMENT", amount: '7500.00' },
            { description: "ADDT'L PROCESSING - IAS CLEARANCE", amount: '8000.00' },
        ];
        const allianceReceiptedTemplates = [
            { description: 'WHARFAGE FEE - PPA SI#', amount: '856.96' },
            { description: 'ARRASTRE CHARGES - ICTSI CSI#' },
            { description: 'CTNR.EQUIPMENT CLEARANCE - AISL OR#' },
            { description: 'FCL CHARGES -' },
            { description: 'DEMURRAGE FEE -' },
            { description: 'TRUCKING/DELIVERY CHARGES - MAMBOG', amount: '23000.00' },
            { description: 'TABS (TERMINAL APPOINTMENT BOOKING  SYSTEM)', amount: '1000.00' },
            { description: 'RETURN OF EMPTY CONTAINER  + EXPENSES', amount: '3500.00' },
        ];
        const linkEnergiesReceiptedTemplates = [
            'WHARFAGE FEE - PPA SI#',
            'ARRASTRE CHARGES - ICTSI CSI#',
            'CTNR.EQUIPMENT CLEARANCE - AISL OR#',
            'HANDLING FEE - WORLDTRANS SI#',
            'DEMURRAGE FEE - TSLINE OR#',
            'TRUCKING/DELIVERY CHARGES - LAS PINAS',
            'TABS (TERMINAL APPOINTMENT BOOKING SYSTEM)',
            'EMPTY RETURN FEE + BOOKING',
        ];
        const petPlusNonReceiptedTemplates = [
            { description: 'CUSTOMS FORMS;STAMPS & NOTARIAL', amount: '1000.00' },
            { description: 'DOCUMENTATION & HANDLING FEE', amount: '1500.00' },
            { description: 'PROCESSING EXPENSES', amount: '3500.00' },
            { description: "ADD'L PROCESSING - ASSESSMENT", amount: '1500.00' },
            { description: "BAI INSPECTOR'S FEE", amount: '1500.00' },
            { description: 'X-RAY FIELD & OTHER EXPENSES', amount: '1500.00' },
        ];
        const petPlusReceiptedTemplates = [
            'WHARFAGE DUE - PPA SI#',
            'ARRASTRE CHARGES -',
            'CTNR. EQUIPMENT CLEARANCE - AISL OR#',
            'FCL CHARGES -',
            'INSPECTION FEE - BAI INV#',
            'TRUCKING /DELIVERY CHARGES -',
            'TABS (TERMINAL APPOINTMENT BOOKING SYSTEM)',
            'EMPTY RETURN FEE',
            "ADDT'L DIESEL COST",
        ];
        const wholeOneYardNonReceiptedTemplates = [
            { description: 'CUSTOMS FORMS, STAMPS, NOTARIAL', amount: '1500.00' },
            { description: 'DOCUMENTATION & HANDLING FEE', amount: '2500.00' },
            { description: 'PROCESSING EXPENSES', amount: '4500.00' },
            { description: 'TRUCKING/DELIVERY CHARGES', amount: '8500.00' },
            { description: "ADDT'L PROCESSING - ASSESSMENT", amount: '3000.00' },
            { description: "ADDT'L PROCESSING - WHSE EXPENSES", amount: '3000.00' },
        ];
        const wholeOneYardReceiptedTemplates = [
            'CFS CHARGES -',
        ];
        const touchcoreNonReceiptedTemplates = [
            { description: 'CUSTOMS FORMS; STAMPS; NOTARIAL', amount: '1500.00' },
            { description: 'DOCUMENTATION & HANDLING', amount: '2500.00' },
            { description: 'PROCESSING EXPENSES', amount: '4500.00' },
            { description: 'TRUCKING/ DELIVERY CHARGES', amount: '4500.00' },
            { description: "ADDT'L PROCESSING - WHSE ; ASSESSMENT", amount: '1500.00' },
        ];
        const touchcoreReceiptedTemplates = [
            'STORAGE FEE -',
        ];
        const kLiftNonReceiptedTemplates = [
            { description: 'CUSTOMS FORMS & STAMPS', amount: '320.00' },
            { description: 'DOCUMENTATION AND PHOTOCOPY', amount: '2500.00' },
            { description: 'NOTARIAL FEE & INTERCOMMERCE CHARGE', amount: '165.00' },
            { description: 'HANDLING FEE', amount: '1500.00' },
            { description: 'PROCESSING EXPENSES', amount: '4500.00' },
            { description: "ADDT'L PROCESSING - ASSESSMENT", amount: '4500.00' },
            { description: "XRAY ; ESS (PHP 2,000.00 x 1 CTNR) + PHP 1,000.00 x 1 CTNR)", amount: '3000.00' },
            { description: "ADDT'L PROCESSING - AOCG+IAS", amount: '8000.00' },
        ];
        const kLiftReceiptedTemplates = [
            'WHARFAGE FEE - CPA INV#',
            'ARRASTRE CHARGES - OPASCOR CI#',
            'FCL CHARGES -',
            'CTNR. EQUIPMENT CLEARANCE - AISL OR#',
        ];
        const bayfieldNonReceiptedTemplates = [
            { description: 'CUSTOMS FORMS & STAMPS', amount: '320.00' },
            { description: 'DOCUMENTATION AND PHOTOCOPY', amount: '2500.00' },
            { description: 'NOTARIAL FEE & INTERCOMMERCE CHARGE', amount: '165.00' },
            { description: 'HANDLING FEE', amount: '1500.00' },
            { description: 'PROCESSING EXPENSES', amount: '4500.00' },
            { description: 'TRUCKING/DELIVERY CHARGES - CARMONA', amount: '9500.00' },
            { description: 'DOE/ATRIG PROCESSING', amount: '3500.00' },
            { description: 'BIR WITHDRAWAL CERTIFICATE PROCESSING', amount: '4500.00' },
            { description: "ADDT'L PROCESSING - WHSE/ NO MARKINGS", amount: '4000.00' },
            { description: "ADDT'L PROCESSING - ASSESSMENT", amount: '6000.00' },
        ];
        const bayfieldReceiptedTemplates = [
            'CFS CHARGES -',
            'LCL CHARGES -',
            'NOTARY',
            'DOE INV#',
        ];
        const sunstateNonReceiptedTemplates = [
            { description: 'CUSTOMS FORMS & STAMPS', amount: '320.00' },
            { description: 'DOCUMENTATION & PHOTOCOPY', amount: '2500.00' },
            { description: 'NOTARIAL FEE & INTERCOMMERCE CHARGE', amount: '165.00' },
            { description: 'HANDLING FEE', amount: '1500.00' },
            { description: 'PROCESSING EXPENSES', amount: '4500.00' },
            { description: 'TRUCKING/DELIVERY CHARGES', amount: '3500.00' },
            { description: "ADDT'L PROCESSING - WHSE EXPENSES", amount: '1500.00' },
        ];
        const sunstateReceiptedTemplates = [
            'CFS CHARGES -',
            'LCL CHARGES -',
        ];
        ...
        const addSiRow = () => {
            ...
            const row = document.createElement('tr');
            row.innerHTML = siRowHtml({});
            body.appendChild(row);
        };

        const isSiSectionHeader = (value) => {
            const upper = String(value || '').trim().toUpperCase();
            return upper === 'A. SERVICE CHARGES' || upper === 'B. OTHER SERVICES OF CUSTOMS BROKER';
        };

        const siRowHtml = ({ description = '', amount = '' } = {}) => {
            const safeDescription = String(description).replace(/"/g, '&quot;');
            if (isSiSectionHeader(description)) {
                return `
                    <td><input class="form-control" name="si_item_description[]" value="${safeDescription}"></td>
                    <input type="hidden" name="si_quantity[]" value="">
                    <input type="hidden" name="si_unit_cost[]" value="">
                    <td>
                        <input type="hidden" name="si_amount[]" value="0">
                        <span class="text-muted small">Section header</span>
                    </td>
            ...
            return `
                ...
                <input type="hidden" name="si_quantity[]" value="">
                <input type="hidden" name="si_unit_cost[]" value="">
                <td><input class="form-control text-end amount-input-si si-amount-input" name="si_amount[]" type="number" step="0.01" min="0" value="${amount}"></td>
                <td class="text-end"><button class="btn btn-sm btn-outline-danger remove-si-row" type="button">&times;</button></td>
            `;
        };

        const ensureSiHeaderRowState = (row) => {
            if (!(row instanceof HTMLElement)) return;
            const descInput = row.querySelector('input[name="si_item_description[]"]');
            if (!(descInput instanceof HTMLInputElement)) return;
            const amountCell = row.children[1];
            if (!(amountCell instanceof HTMLTableCellElement)) return;

            const amountInput = amountCell.querySelector('input[name="si_amount[]"]');
            const currentlyHeader = !!(amountInput && amountInput.type === 'hidden');
            const shouldBeHeader = isSiSectionHeader(descInput.value);
            if (currentlyHeader === shouldBeHeader) return;

            if (shouldBeHeader) {
                amountCell.innerHTML = '<input type="hidden" name="si_amount[]" value="0"><span class="text-muted small">Section header</span>';
                return;
            }

            amountCell.innerHTML = '<input class="form-control text-end amount-input-si si-amount-input" name="si_amount[]" type="number" step="0.01" min="0" value="">';
        };

        const syncAllSiHeaderRows = () => {
            const body = document.getElementById('siItemsBody');
            if (!body) return;
            body.querySelectorAll('tr').forEach((row) => ensureSiHeaderRowState(row));
        };
        ...
        const isKiltonClark = () => {
            const billTo = document.getElementById('billToInput')?.value || '';
            const busStyle = document.getElementById('businessStyleInput')?.value || '';
            const billingAddress = document.getElementById('billAddressInputBilling')?.value || '';
            const serviceAddress = document.getElementById('billAddressInputService')?.value || '';
            const clientText = `${billTo} ${busStyle}`.toUpperCase();
            const addressText = `${billingAddress} ${serviceAddress}`.toUpperCase();

            return clientText.includes('KILTON') && addressText.includes('CLARK');
        };
        ...
        const isAlliance = () => {
            ...
            return `${billTo} ${busStyle}`.toUpperCase().includes('ALLIANCE');
        };
        ...
        const isPetPlus = () => {
            ...
            const busStyle = document.getElementById('businessStyleInput')?.value || '';
            const text = `${billTo} ${busStyle}`.toUpperCase();
            return text.includes('PET PLUS');
        };

        const isWholeOneYard = () => {
            const billTo = document.getElementById('billToInput')?.value || '';
            const busStyle = document.getElementById('businessStyleInput')?.value || '';
            const text = `${billTo} ${busStyle}`.toUpperCase();
            return text.includes('WHOLE ONE YARD') || text.includes('WHOLEONEYARD');
        };
        ...
        const isTouchcore = () => {
            ...
            const busStyle = document.getElementById('businessStyleInput')?.value || '';
            const text = `${billTo} ${busStyle}`.toUpperCase();
            return text.includes('TOUCHCORE');
        };
        ...
        const seedDescriptionRows = (tbodyId, descriptionName, amountName, templates) => {
            const body = document.getElementById(tbodyId);
            if (!body) {
                return;
            }

            const existingDescs = [...body.querySelectorAll(`input[name="${descriptionName}"]`)];
            const hasNonEmpty = existingDescs.some((input) => (input.value || '').trim() !== '');
            if (hasNonEmpty) {
                return;
            }
            ...
            templates.forEach((templateRow) => {
                const desc = typeof templateRow === 'string' ? templateRow : (templateRow.description || '');
                const amount = typeof templateRow === 'string' ? '' : (templateRow.amount || '');
                const row = document.createElement('tr');
                row.innerHTML = `
                    <td><input class="form-control" name="${descriptionName}" value="${desc.replace(/"/g, '&quot;')}"></td>
                    <td><input class="form-control text-end amount-input" name="${amountName}" type="number" step="0.01" min="0" value="${amount}"></td>
                    <td class="text-end">
                        <button class="btn btn-sm btn-outline-danger remove-row" type="button">&times;</button>
                    </td>
                `;
        ...
        const applyKiltonClarkTemplate = () => {
            seedDescriptionRows('nonReceiptedBody', 'non_receipted_desc[]', 'non_receipted_amount[]', kiltonClarkNonReceiptedTemplates);
            seedDescriptionRows('receiptedBody', 'receipted_desc[]', 'receipted_amount[]', kiltonClarkReceiptedTemplates);
            const siBody = document.getElementById('siItemsBody');
            if (!siBody) {
                return;
            }
            const existingSiDescs = [...siBody.querySelectorAll('input[name="si_item_description[]"]')];
            const hasCustomSiRows = existingSiDescs.some((input) => {
                const value = (input.value || '').trim();
                return value !== '' && value !== 'BROKERAGE FEE AS PER CAO 1-2001';
            });
            if (hasCustomSiRows) {
                return;
            }
            siBody.innerHTML = '';
            kiltonClarkServiceTemplates.forEach((desc) => {
                const row = document.createElement('tr');
                row.innerHTML = siRowHtml({ description: desc });
                siBody.appendChild(row);
            });
        };

        const applyAllianceTemplate = () => {
            seedDescriptionRows('nonReceiptedBody', 'non_receipted_desc[]', 'non_receipted_amount[]', allianceNonReceiptedTemplates);
            seedDescriptionRows('receiptedBody', 'receipted_desc[]', 'receipted_amount[]', allianceReceiptedTemplates);
        };

        const applyLinkEnergiesTemplate = () => {
            seedDescriptionRows('receiptedBody', 'receipted_desc[]', 'receipted_amount[]', linkEnergiesReceiptedTemplates);
        };

        const applyPetPlusTemplate = () => {
            seedDescriptionRows('nonReceiptedBody', 'non_receipted_desc[]', 'non_receipted_amount[]', petPlusNonReceiptedTemplates);
            seedDescriptionRows('receiptedBody', 'receipted_desc[]', 'receipted_amount[]', petPlusReceiptedTemplates);
        };

        const applyWholeOneYardTemplate = () => {
            seedDescriptionRows('nonReceiptedBody', 'non_receipted_desc[]', 'non_receipted_amount[]', wholeOneYardNonReceiptedTemplates);
            seedDescriptionRows('receiptedBody', 'receipted_desc[]', 'receipted_amount[]', wholeOneYardReceiptedTemplates);
        };

        const applyTouchcoreTemplate = () => {
            seedDescriptionRows('nonReceiptedBody', 'non_receipted_desc[]', 'non_receipted_amount[]', touchcoreNonReceiptedTemplates);
            seedDescriptionRows('receiptedBody', 'receipted_desc[]', 'receipted_amount[]', touchcoreReceiptedTemplates);
        };

        const applyKLiftTemplate = () => {
            seedDescriptionRows('nonReceiptedBody', 'non_receipted_desc[]', 'non_receipted_amount[]', kLiftNonReceiptedTemplates);
            seedDescriptionRows('receiptedBody', 'receipted_desc[]', 'receipted_amount[]', kLiftReceiptedTemplates);
        };

        const applyBayfieldTemplate = () => {
            seedDescriptionRows('nonReceiptedBody', 'non_receipted_desc[]', 'non_receipted_amount[]', bayfieldNonReceiptedTemplates);
            seedDescriptionRows('receiptedBody', 'receipted_desc[]', 'receipted_amount[]', bayfieldReceiptedTemplates);
        };

        const applySunstateTemplate = () => {
            seedDescriptionRows('nonReceiptedBody', 'non_receipted_desc[]', 'non_receipted_amount[]', sunstateNonReceiptedTemplates);
            seedDescriptionRows('receiptedBody', 'receipted_desc[]', 'receipted_amount[]', sunstateReceiptedTemplates);
        };
        ...
        const applyClientMode = () => {
            if (isConverge()) {
                ...
                if (isPetPlus()) {
                    applyPetPlusTemplate();
                } else if (isWholeOneYard()) {
                    applyWholeOneYardTemplate();
                } else if (isTouchcore()) {
                    applyTouchcoreTemplate();
                } else if (isKLift()) {
                    applyKLiftTemplate();
                } else if (isBayfield()) {
                    applyBayfieldTemplate();
                } else if (isSunstate()) {
                    applySunstateTemplate();
                } else if (isLinkEnergies()) {
                    applyLinkEnergiesTemplate();
                } else if (isAlliance()) {
                    applyAllianceTemplate();
                } else if (isKiltonClark()) {
                    applyKiltonClarkTemplate();
                }
            }
            refreshTotals();
        };
        ...
        const refreshTotals = () => {
            ...
            if (isService) {
                const siRows = [...document.querySelectorAll('#siItemsBody tr')];
                const rowData = siRows.map((row) => {
                    const desc = (row.querySelector('input[name="si_item_description[]"]')?.value || '').trim();
                    const amountEl = row.querySelector('input[name="si_amount[]"]');
                    const amount = parseFloat(amountEl?.value || '0') || 0;
                    return { desc, amount };
                });

                const hasManualTaxRows = rowData.some(({ desc }) => /(VAT|EWT|WITHHOLDING)/i.test(desc));
                const vatRowsTotal = rowData
                    .filter(({ desc }) => /VAT/i.test(desc))
                    .reduce((sum, { amount }) => sum + amount, 0);
                const withholdingRowsTotalAbs = Math.abs(
                    rowData
                        .filter(({ desc }) => /(EWT|WITHHOLDING)/i.test(desc))
                        .reduce((sum, { amount }) => sum + amount, 0)
                );

                const netVat = hasManualTaxRows ? siAmount : siAmount;
                const vat = hasManualTaxRows ? vatRowsTotal : (netVat * 0.12);
                const totalSales = hasManualTaxRows ? siAmount : (netVat + vat);
                const applyWithholding = document.getElementById('siApplyWithholdingTax')?.checked;
                const withholding = hasManualTaxRows
                    ? withholdingRowsTotalAbs
                    : (applyWithholding ? (netVat * 0.10) : 0);
                const totalDue = hasManualTaxRows ? siAmount : (totalSales - withholding);
        ...
        billingForm?.addEventListener('submit', (event) => {
            ...
            const isDraftAction = submitter?.id === 'draftDocumentBtn';
            ...
            if (isDraftAction) {
                setConnectivityBanner(false, 'Offline: Draft print requires internet. Please reconnect, then click Print Draft again.');
                return;
            }

            const payload = {
                action: billingForm.action,
                method: billingForm.method || 'POST',
                target: billingForm.target || '',
                formData: captureFormSnapshot(billingForm),
                queuedAt: new Date().toISOString(),
            };
            setQueuedPayload(payload);
            setConnectivityBanner(false, 'Offline: Save queued. It will auto-submit once internet is back.');
        });
        ...
        document.addEventListener('input', (event) => {
            ...
            if (event.target.name === 'si_item_description[]') {
                const row = event.target.closest('tr');
                ensureSiHeaderRowState(row);
                refreshTotals();
            }
        });
        ...
        document.getElementById('billAddressInputBilling')?.addEventListener('input', applyClientMode);
        document.getElementById('billAddressInputService')?.addEventListener('input', applyClientMode);
        ...
        document.getElementById('btnServiceInvoice')?.addEventListener('click', (event) => {
            const joId = document.querySelector('input[name="job_order_id"]')?.value || '';
            const target = '{{ route('billing.service-invoices.create') }}' + (joId ? ('?job_order_id=' + encodeURIComponent(joId)) : '');
            window.location.href = target;
        });

        window.addEventListener('online', () => {
            setConnectivityBanner(true);
            submitQueuedWhenOnline();
        });
        ...
        const initialQueued = getQueuedPayload();
        if (initialQueued) {
            restoreFormSnapshot(billingForm, initialQueued.formData);
            setConnectivityBanner(navigator.onLine, navigator.onLine
                ? 'Online: Reconnected. Submitting your queued save now...'
                : 'Offline: Queued save detected. It will submit automatically when internet is back.');
        } else {
            setConnectivityBanner(navigator.onLine);
        }
        if (navigator.onLine) {
            submitQueuedWhenOnline();
        }
